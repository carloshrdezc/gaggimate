...
// ---- platform sockets: POSIX (macOS/Linux) vs Winsock2 (Windows/MinGW) -----
#ifdef _WIN32
// Winsock2 must come before windows.h; ws2tcpip pulls in TCP_NODELAY/inet_pton.
#include <winsock2.h>
#include <ws2tcpip.h>

// Map the BSD socket vocabulary the body uses onto the Winsock equivalents so the
// rest of this file is identical on both platforms.
#define SIM_SOCK_CLOSE(fd) ::closesocket(fd)
// WSAPoll has the same shape as poll(); pollfd/POLLIN are provided by winsock2.
#define poll(fds, n, to) WSAPoll((fds), (n), (to))

// Winsock reports per-call errors via WSAGetLastError(), not errno.
static inline int simSockErr() { return WSAGetLastError(); }
#define SIM_EWOULDBLOCK WSAEWOULDBLOCK
#define SIM_EAGAIN WSAEWOULDBLOCK
#define SIM_EINTR WSAEINTR

// Put a socket into non-blocking mode (no fcntl on Winsock).
static inline void simSetNonBlocking(SOCKET fd) {
    u_long mode = 1;
    ::ioctlsocket(fd, FIONBIO, &mode);
}

// Process-wide Winsock init/teardown (idempotent; runs before main via static ctor).
namespace {
struct WinsockInit {
    WinsockInit() {
        WSADATA wsa;
...
#define SIM_SOCK_CLOSE(fd) ::close(fd)
static inline int simSockErr() { return errno; }
#define SIM_EWOULDBLOCK EWOULDBLOCK
#define SIM_EAGAIN EAGAIN
#define SIM_EINTR EINTR

// Put a socket into non-blocking mode.
static inline void simSetNonBlocking(int fd) {
...
void sendAll(int fd, const void *data, size_t len) {
    ...
    while (sent < len) {
        ...
        if (n > 0) {
            sent += n;
        } else if (n < 0 && (simSockErr() == SIM_EAGAIN || simSockErr() == SIM_EWOULDBLOCK || simSockErr() == SIM_EINTR)) {
            continue;
        } else {
            break;
        }
...
void AsyncWebSocket::textAll(const String &message) {
    for (auto *c : _clients)
        sendWsFrame(c->fd(), WS_TEXT, (const uint8_t *)message.c_str(), message.length());
}
...
// ---- response helpers ------------------------------------------------------
// Map a file extension to a Content-Type so the browser renders the WebUI
// correctly (the device's serveStatic does the same).
static String guessContentType(const std::string &path) {
...
void AsyncWebServer::begin() {
    if (_listenFd >= 0)
        return;
    uint16_t preferred = _port < 1024 ? 8080 : _port; // avoid needing root for :80
    _listenFd = (int)socket(AF_INET, SOCK_STREAM, 0);
    if (_listenFd < 0) {
        fprintf(stderr, "[sim-web] socket() failed: %d\n", simSockErr());
        return;
    }
    int yes = 1;
    setsockopt(_listenFd, SOL_SOCKET, SO_REUSEADDR, (const char *)&yes, sizeof(yes));
    simSetNonBlocking(_listenFd);

    // Try the preferred port first, then a small range — on Windows another
    // process holding the port with exclusive use makes bind() fail with
    // WSAEACCES(10013), and dev machines often already have 8080 occupied.
    uint16_t port = 0;
    for (uint16_t p = preferred; p <= preferred + 10; p++) {
        sockaddr_in addr{};
    ...
    if (port == 0) {
        fprintf(stderr, "[sim-web] failed to bind %u-%u (last err %d)\n", preferred, (uint16_t)(preferred + 10), simSockErr());
        SIM_SOCK_CLOSE(_listenFd);
        _listenFd = -1;
        return;
    }
    ...
    for (size_t i = 0; i < _conns.size();) {
        ...
        while (true) {
            ...
            if (n > 0) {
                ...
                if (simSockErr() == SIM_EAGAIN || simSockErr() == SIM_EWOULDBLOCK)
                    break;
                dead = true;
                break;
            }
...
void AsyncWebServer::dispatch(Conn &c, AsyncWebServerRequest &req) {
    ...
    for (auto &s : _static) {
        if (path.rfind(s.uri, 0) == 0) { // prefix match
            ...
            // Directory or root request -> serve the default index.html.
            if (file.empty() || file.back() == '/')
                file += "index.html";
            else if (rel.empty())
                file += "/index.html";
            ...
            if (gz) {
                ...
                uint8_t chunk[4096];
                size_t n;
                while ((n = gz.read(chunk, sizeof(chunk))) > 0)
                    r._body.append((const char *)chunk, n);
                gz.close();
                writeResponse(c.fd, &r);
                return;
            }
            req.send(*s.fs, String(file.c_str()), contentType);
            return;
        }
    }
    if (_notFound)
