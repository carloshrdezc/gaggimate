...
#include <direct.h>
#include <io.h>
// Windows _mkdir() takes no mode argument; ignore the POSIX mode.
static inline int simMkdir(const char *path, int /*mode*/) { return ::_mkdir(path); }
#else
#include <unistd.h>
static inline int simMkdir(const char *path, int mode) { return ::mkdir(path, mode); }
#endif
...
bool LittleFSFS::begin(bool, const char *, uint8_t, const char *) {
    ...
    simMkdir("sim_data", 0755);
    simMkdir(_root.c_str(), 0755);
    return true;
}
