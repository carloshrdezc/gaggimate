...
// Windows/MinGW lacks a few POSIX libc functions the firmware uses (localtime_r,
// setenv/unsetenv). Provide thin C++-only shims so the firmware sources compile
// unchanged under GAGGIMATE_SIM. Guarded to C++ so the C sources (LVGL) that pull
// this umbrella for millis() are unaffected. [CAR-399]
...
inline int setenv(const char *name, const char *value, int /*overwrite*/) { return _putenv_s(name, value ? value : ""); }
...
#ifdef _WIN32
    localtime_s(info, &now); // Windows: reversed arg order vs POSIX localtime_r
#else
