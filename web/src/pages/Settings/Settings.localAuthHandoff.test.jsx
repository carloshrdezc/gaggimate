...
const { authenticatedFetch, localAuthHandoffUrl, fetchedSettings } = vi.hoisted(() => ({
  ...
  localAuthHandoffUrl: vi.fn(
    hostname => `http://${hostname}.local/#localAuthToken=bootstrap-token`,
  ),
  fetchedSettings: {
...
vi.mock('../../services/localAuthFetch.js', () => ({
  ...
  localAuthHandoffUrl,
  MDNS_NAME_ERROR:
    'Hostname must be 1-63 ASCII letters, digits, or hyphens, without a leading or trailing hyphen.',
}));

vi.mock('../../components/Card.jsx', () => ({
  default: ({ children }) => h('section', {}, children),
}));
...
test.each([
  ...
  fireEvent.click(
    screen.getByRole('button', { name: 'Provision Wi-Fi and copy auth handoff link' }),
  );
...
test('provisions only AP Wi-Fi fields, then copies and renders the matching handoff URL', async () => {
  ...
  expect(
    screen.getByText('http://new-gaggimate.local/#localAuthToken=bootstrap-token'),
  ).toBeTruthy();
});
...
test.each([
  ...
  await waitFor(() =>
    expect(screen.getByRole('alert').textContent).toBe(
      'Wi-Fi provisioning failed. Check the connection and try again.',
    ),
  );
  expect(navigator.clipboard.writeText).not.toHaveBeenCalled();
  expect(screen.queryByText('http://gaggimate.local/#localAuthToken=bootstrap-token')).toBeNull();
  expect(
    screen.queryByText(/The device restarts into STA after this successful provisioning/i),
  ).toBeNull();
});
