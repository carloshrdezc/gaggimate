...
- **This is the single most dangerous user-facing risk.** Mitigations (pick one, design in the OTA/migration PR):
  ...
  - **(c) Off-device export/import (loud warning + export-first UX):** the user exports profiles OFF the device via the web UI *before* flashing; the new firmware clean-formats LittleFS and seeds the Default profile; the user re-imports. Document that this specific upgrade resets profiles/history; make the export step impossible to miss.
  - **⚠️ SUPERSEDED RECOMMENDATION (2026-06-18, PRO-218).** This spike originally recommended **(b)** the in-place RAM-staging shim. **That recommendation is now REJECTED.** The shim was implemented (PR #200), passed 72/72 host tests and two reviews, then **WIPED ALL 7 PROFILES on a real-hardware upgrade test.** Root cause: single-partition + RAM staging makes the only copy volatile; the on-device SPIFFS rescue read returned empty (unlike the host mock); and the "verified restore" gate passed vacuously (0 staged == 0 restored == "success"), so the once-only marker got stamped over wiped data. Mock-FS host tests are structurally incapable of catching this. Industry precedent (ESPEasy, same SPIFFS→LittleFS problem at scale) confirms in-place RAM staging is an anti-pattern and ships **off-device backup/restore** instead.
  - **✅ ADOPTED (Carlos's decision, PRO-218): Option (c) — off-device export/import.** Because the data lives off-device across the format boundary, a silent on-device wipe is structurally impossible. GaggiMate's web UI already round-trips all of `/p` via the **Export Profiles / Import Profiles** controls on the Profiles page (bulk `profiles.json` export + collision-remapping import). The firmware-side change is minimal: confirm the clean-format path (`LittleFS.begin(true, ...)` → `ProfileManager::setup()` → `migrate()` seeds Default) is safe, and add a **prominent pre-upgrade warning** (banner on the System & Updates / OTA page) telling users to export first. Do **not** resurrect the in-place shim.
...
- Survival across the upgrade = the §4.2 mitigation, now **Option (c) off-device export/import** (in-place shim rejected after PR #200's hardware wipe). `/p` round-trips through the web UI's bulk Export/Import; `/h` (shot history) is **not** preserved by the profile export and is treated as expendable across this one boundary (document this). After that boundary, profiles persist across all future OTAs (the win).
...
8. **PRO-218 — OTA migration & data-preservation across the SPIFFS→LittleFS boundary** *(firmware + web — highest user-facing risk)*
   **Adopt §4.2 Option (c): off-device export/import** (the in-place RAM-staging shim from §4.2(b) was implemented in PR #200 and REJECTED after it wiped all 7 profiles on real hardware — see the SUPERSEDED note in §4.2). The web UI already bulk-exports/imports `/p`; confirm the clean-format-on-upgrade path seeds Default safely and add a prominent pre-upgrade export warning. Field-test an upgrade from a real SPIFFS device (export → flash → import → reboot). **Ship last, gate the release on it** — this is the difference between "OTA stops wiping data" and "the upgrade wipes everyone's data once."
