...
- Web assets are embedded into the firmware app image: run `scripts/build_webui.sh` (gzips + packs the bundle into `src/display/webassets/`) before `pio run`. The fresh-install filesystem image now holds only seed profiles (`data/p`).
